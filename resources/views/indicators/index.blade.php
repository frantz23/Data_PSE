...
<div class="d-flex justify-content-end">
    ...
    <a href="{{ route('admin.indicator.create') }}" class="btn btn-success m-1">
        ...
        <table  id="Indicator" class="table">
            <thead>
                <tr>
                    <th scope="col">N#</th>
						...
						<th scope="col">Result_level</th>
						<th scope="col">Data_type</th>
						<th scope="col">Unit</th>
						<th scope="col">Baseline</th>
						<th scope="col">Target</th>
						<th scope="col">Current_value</th>
            ...
            <tbody>
                @foreach($indicators as $indicator)
						...
						<td>
                    <a href="{{ route('admin.indicator.show', ['id' => $indicator->id]) }}" class="btn btn-primary btn-sm">
                        <i class="fa-solid fa-eye"></i>
                    </a>
                    <a href="{{ route('admin.indicator.edit', ['id' => $indicator->id]) }}" class="btn btn-success btn-sm">
                        <i class="fa-solid fa-pen-to-square"></i>
                    </a>
                    <a href="#" data-id="{{ $indicator->id }}" class="btn btn-danger btn-sm deleteBtn">
...
@section('scripts')
    ...
    <script>
        ...
        checkbox.onchange = async (event) => {
            ...
            const response = await fetch('/admin/indicators/speed/' + id, {
                method: 'PUT',
        ...
        deleteButtons.forEach(deleteButton => {
            deleteButton.addEventListener('click', (event)=>{
                ...
                confirmDeleteBtn.addEventListener('click',async ()=>{
                    const csrfToken = document.head.querySelector('meta[name="csrf-token"]').content;
                    const response = await fetch('/admin/indicators/delete/'+id , {
                        method: 'DELETE',
                        headers: {
                            'Content-Type': 'application/json',
                            'X-CSRF-TOKEN': csrfToken
