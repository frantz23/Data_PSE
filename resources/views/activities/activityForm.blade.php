    ...
    <div class="row">
        <div class="col-md-8">
            <form action="{{ isset($activity) ? route('admin.activity.update', ['activity' => $activity->id]) : route('admin.activity.store') }}" method="POST">
                @csrf
                @if(isset($activity))
                @method('PUT')
                @endif
                ...
                <a href="{{ route('admin.activity.index') }}" class="btn btn-danger mt-1">
                    Cancel
                </a>
