...
@section('content')
<div >
<h3> Indicatorvaluefiles Details</h3>
...
<div class="d-flex justify-content-end">
    ...
    <a href="{{ route('admin.indicatorvaluefile.create') }}" class="btn btn-success m-1">

            Create Indicatorvaluefile
...
<div class="">
    ...
    <div class="table-responsive">
        <table  id="Indicatorvaluefile" class="table">
            <thead>
                <tr>
                    <th scope="col">N#</th>
            ...
            <tbody>
                @foreach($indicatorvaluefiles as $indicatorvaluefile)
						<tr><td>{{ $indicatorvaluefile->id }}</td>
						...
						<td>
                    <a href="{{ route('admin.indicatorvaluefile.show', ['id' => $indicatorvaluefile->id]) }}" class="btn btn-primary btn-sm">
                        <i class="fa-solid fa-eye"></i>
                    </a>
                    <a href="{{ route('admin.indicatorvaluefile.edit', ['id' => $indicatorvaluefile->id]) }}" class="btn btn-success btn-sm">
                        <i class="fa-solid fa-pen-to-square"></i>
                    </a>
                    <a href="#" data-id="{{ $indicatorvaluefile->id }}" class="btn btn-danger btn-sm deleteBtn">
        ...
        <div class="d-flex justify-content-center">
            {{ $indicatorvaluefiles->links('pagination::bootstrap-5') }}
        </div>
    </div>
</div>
</div>
...
@section('scripts')
    ...
    <script>
        ...
        checkbox.onchange = async (event) => {
            ...
            const response = await fetch('/admin/indicatorvaluefiles/speed/' + id, {
                method: 'PUT',
        ...
        deleteButtons.forEach(deleteButton => {
            deleteButton.addEventListener('click', (event)=>{
                ...
                confirmDeleteBtn.addEventListener('click',async ()=>{
                    const csrfToken = document.head.querySelector('meta[name="csrf-token"]').content;
                    const response = await fetch('/admin/indicatorvaluefiles/delete/'+id , {
                        method: 'DELETE',
                        headers: {
                            'Content-Type': 'application/json',
                            'X-CSRF-TOKEN': csrfToken
        ...
        document.addEventListener('DOMContentLoaded', function() {
            const tableHeaders = document.querySelectorAll('#Indicatorvaluefile th');
            ...
            tableHeaders.forEach(function(header, index) {
                ...
                const savedSelection = localStorage.getItem('selectedColumns#Indicatorvaluefile');
        ...
        function toggleColumn(columnIndex, show) {
            const dataTable = document.getElementById('Indicatorvaluefile');
        ...
        function saveSelection() {
            ...
            localStorage.setItem('selectedColumns#Indicatorvaluefile', JSON.stringify(selectedColumns));
        }
        ...
        function sortTable(columnIndex) {
            const table = document.getElementById('Indicatorvaluefile');
