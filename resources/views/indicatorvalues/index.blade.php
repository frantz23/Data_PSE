...
<div class="d-flex justify-content-end">
    ...
    <a href="{{ route('admin.indicatorvalue.create') }}" class="btn btn-success m-1">
...
<div class="">
    ...
    <div class="table-responsive">
        <table  id="Indicatorvalue" class="table">
            <thead>
                <tr>
                    <th scope="col">N#</th>
						<th scope="col">Value_numeric</th>
						<th scope="col">Value_text</th>
            ...
            <tbody>
                @foreach($indicatorvalues as $indicatorvalue)
						...
						<td>
                    <a href="{{ route('admin.indicatorvalue.show', ['id' => $indicatorvalue->id]) }}" class="btn btn-primary btn-sm">
                        <i class="fa-solid fa-eye"></i>
                    </a>
                    <a href="{{ route('admin.indicatorvalue.edit', ['id' => $indicatorvalue->id]) }}" class="btn btn-success btn-sm">
                        <i class="fa-solid fa-pen-to-square"></i>
                    </a>
                    <a href="#" data-id="{{ $indicatorvalue->id }}" class="btn btn-danger btn-sm deleteBtn">
        ...
        <div class="d-flex justify-content-center">
            {{ $indicatorvalues->links('pagination::bootstrap-5') }}
        </div>
    </div>
</div>
</div>
...
@section('scripts')
    ...
    <script>
        ...
        checkbox.onchange = async (event) => {
            ...
            const response = await fetch('/admin/indicatorvalues/speed/' + id, {
                method: 'PUT',
        ...
        deleteButtons.forEach(deleteButton => {
            deleteButton.addEventListener('click', (event)=>{
                ...
                confirmDeleteBtn.addEventListener('click',async ()=>{
                    const csrfToken = document.head.querySelector('meta[name="csrf-token"]').content;
                    const response = await fetch('/admin/indicatorvalues/delete/'+id , {
                        method: 'DELETE',
                        headers: {
                            'Content-Type': 'application/json',
                            'X-CSRF-TOKEN': csrfToken
