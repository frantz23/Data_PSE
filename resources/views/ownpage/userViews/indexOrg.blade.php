    ...
    <div class="container py-4">
        ...
        <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center mb-4 gap-3">
            ...
            <div class="d-flex align-items-center gap-2">
                ...
                <a href="{{ route('assignViewRole') }}" class="btn btn-outline-warning rounded-3 shadow-sm">
                    <i class="bi bi-person-plus-fill me-1"></i> Assigner un rôle
                </a>
            </div>
        </div>
                    ...
                    <table id="User" class="table table-hover align-middle mb-0">
                        <thead class="bg-light border-bottom">
                            <tr class="text-secondary small text-uppercase fw-semibold" style="cursor: pointer;">
                                <th scope="col" class="ps-4">N# <i
                                        class="bi bi-arrow-down-up ms-1 text-muted small"></i></th>
                                ...
                                <th scope="col">Mot de passe <i class="bi bi-arrow-down-up ms-1 text-muted small"></i>
                                </th>
                                <th scope="col">Organisation <i class="bi bi-arrow-down-up ms-1 text-muted small"></i>
                                </th>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        @if (isset($user->organization))
                                            <span class="badge bg-light text-dark border px-2 py-1 rounded-pill">
                                                <i
                                                    class="bi bi-building me-1 text-primary"></i>{{ $user->organization->name }}
                                            </span>
                                        @else
                                    ...
                                    <td class="text-end pe-4">
                                        <div class="btn-group" role="group">
                                            <a href="{{ route('showUserOrg', ['id' => $user->id]) }}"
                                                class="btn btn-sm btn-outline-primary" title="Voir les détails">
                                                <i class="bi bi-eye"></i>
                                            </a>
                                            <a href="{{ route('editUserOrg', ['id' => $user->id]) }}"
                                                class="btn btn-sm btn-outline-success" title="Modifier">
                                                <i class="bi bi-pencil-square"></i>
                                            </a>
                                            <a href="#" data-id="{{ $user->id }}"
                                                class="btn btn-sm btn-outline-danger deleteBtn" title="Supprimer">
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.querySelectorAll('.deleteBtn').forEach(button => {
                button.addEventListener('click', function(e) {
                    ...
                    if (modalBody) {
                        modalBody.innerHTML =
                            `Êtes-vous sûr de vouloir supprimer l'utilisateur <strong>#${userIdToDelete}</strong> ? Cette action est irréversible.`;
                    }
            ...
            if (confirmDeleteBtn) {
                confirmDeleteBtn.addEventListener('click', async function() {
                    ...
                    if (!csrfMeta) {
                        alert(
                            "Erreur : Le token CSRF est introuvable dans le header <head> de votre layout.");
                        return;
                    }
            ...
            if (columnSelector) {
                tableHeaders.forEach(function(header, index) {
                    ...
                    div.className =
                        'form-check form-switch m-0 d-flex justify-content-between align-items-center';
