...
<div class="container-fluid py-4 px-4">
            ...
            <div class="d-flex justify-content-between align-items-start flex-wrap gap-3">
                <div>
                    <div class="d-flex align-items-center gap-2 mb-2">
                        <span class="badge bg-primary bg-opacity-10 text-primary fw-bold">PROJET #{{ $project->code }}</span>
                        <span class="badge bg-secondary text-capitalize">{{ $project->program->name ?? 'Programme N/A' }}</span>
                ...
                <div class="d-flex gap-2">
                    <a href="{{ route('showProject', $project->id) }}" class="btn btn-outline-secondary rounded-pill px-3">
            ...
            <div class="card border-0 shadow-sm h-100">
                <div class="card-body p-3 d-flex align-items-center justify-content-between">
                    <div>
                        ...
                        <h3 class="fw-bold text-dark mb-0 mt-1">{{ $completedActivities }} / {{ $totalActivities }}</h3>
                    </div>
                ...
                <div class="card-body p-3 d-flex align-items-center justify-content-between">
                    <div>
                        ...
                        <h3 class="fw-bold text-dark mb-0 mt-1">
                            {{ number_format($project->budget ?? 0, 0, ',', ' ') }} <small class="fs-6">FCFA</small>
                        </h3>
                    </div>
    ...
    <div class="row g-4 mb-4">
        ...
        <div class="col-lg-7">
            <div class="card border-0 shadow-sm h-100">
                <div class="card-header bg-white border-0 py-3">
                    <h5 class="fw-bold mb-0 text-dark"><i class="bi bi-clock-history me-2 text-primary"></i>Dernières Collectes de Données</h5>
                </div>
                        ...
                        <tbody>
                            @forelse($latestValues as $val)
                                <tr>
                                    ...
                                    <td>{{ $val->value_numeric ?? $val->value_text }} {{ $val->indicator->unit ?? '' }}</td>
                                    <td>{{ \Carbon\Carbon::parse($val->reporting_date)->format('d/m/Y') }}</td>
                                    <td>
                                        <span class="badge {{ $val->validated ? 'bg-success' : 'bg-warning text-dark' }}">
                                            {{ $val->validated ? 'Validé' : 'En attente' }}
                                        </span>
                                    </td>
                                </tr>
                            @empty
                                <tr><td colspan="4" class="text-center text-muted py-3">Aucune collecte récente.</td></tr>
                            @endforelse
                        </tbody>
                    </table>
                </div>
        ...
        <div class="col-lg-5">
            <div class="card border-0 shadow-sm h-100">
                <div class="card-header bg-white border-0 py-3">
                    <h5 class="fw-bold mb-0 text-dark"><i class="bi bi-file-earmark-check me-2 text-success"></i>Derniers Justificatifs Déposés</h5>
                </div>
                                ...
                                <div class="d-flex align-items-center gap-2">
                                    ...
                                    <div>
                                        ...
                                        <small class="text-muted d-block">
                                            {{ $file->indicatorValue->indicator->code ?? 'N/A' }} • {{ $file->created_at->diffForHumans() }}
                                        </small>
                                    </div>
                                </div>
                                <a href="{{ Storage::url($file->path) }}" target="_blank" class="btn btn-sm btn-light border">
