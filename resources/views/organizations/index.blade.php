...
<div class="d-flex justify-content-end">
    ...
    <a href="{{ route('admin.organization.create') }}" class="btn btn-success m-1">
        ...
        <table  id="Organization" class="table">
            ...
            <tbody>
                @foreach($organizations as $organization)
						...
						<td>
                    <a href="{{ route('admin.organization.show', ['id' => $organization->id]) }}" class="btn btn-primary btn-sm">
                        <i class="fa-solid fa-eye"></i>
                    </a>
                    <a href="{{ route('admin.organization.edit', ['id' => $organization->id]) }}" class="btn btn-success btn-sm">
...
@section('scripts')
    ...
    <script>
        ...
        checkbox.onchange = async (event) => {
            ...
            const response = await fetch('/admin/organizations/speed/' + id, {
                method: 'PUT',
        ...
        deleteButtons.forEach(deleteButton => {
            deleteButton.addEventListener('click', (event)=>{
                ...
                confirmDeleteBtn.addEventListener('click',async ()=>{
                    const csrfToken = document.head.querySelector('meta[name="csrf-token"]').content;
                    const response = await fetch('/admin/organizations/delete/'+id , {
                        method: 'DELETE',
                        headers: {
                            'Content-Type': 'application/json',
                            'X-CSRF-TOKEN': csrfToken
